...
Run `aics -h` for a summary, `aics --help` for full command-line help, or
`aics --manpage` to print this reference manual directly from the CLI.
...
## Automation and AI agent guidelines

By default, `aics` launches an interactive terminal UI (TUI). When stdout is not
a terminal (e.g. in automated scripts, subagents, or shell pipelines), you must
pass `--json` for streaming JSONL search results or `--export DIR` to export Markdown
transcripts. Redirecting or piping without one of these flags is rejected.

### Side-effect and safety matrix

| Command / Flag | Primary End Result | Filesystem Side-Effects / Mutations | Safe for Read-Only Automation? |
| --- | --- | --- | --- |
| `aics [QUERY]` | Launches interactive TUI | None (updates index cache if needed) | No (requires interactive TTY) |
| `aics --json [FILTERS] [QUERY]` | Streams JSONL records to stdout | None (updates index cache if needed) | **Yes** (primary automation command) |
| `aics --export DIR [FILTERS]` | Writes Markdown files to DIR | Creates directory and writes Markdown files | Modifies filesystem (writes files) |
| `aics --preview-rules` | Reviews proposed JS rule actions | None (evaluates rules read-only) | **Yes** (when combined with `--json`) |
| `aics --apply-rules` | Evaluates and executes JS rules | Mutates session files (trash, delete, tag) | **Mutating** (alters session files) |
| `aics --rebuild-index` | Rebuilds search index and continues | Rewrites index cache files | Safe (re-indexes existing files) |
| `aics --delete-index` | Deletes index profile and exits | Deletes index cache directory | Clears local cache |
| `aics --write-rules-dts` | Generates TypeScript declarations | Writes `~/.config/aics/rules.d.ts` | Writes configuration file |
| `aics --print-palettes` | Prints theme color cards to stdout | None | **Yes** (informational) |
| `aics --manpage` | Prints this reference manual to stdout | None | **Yes** (informational) |
...
| Option | Meaning |
| --- | --- |
| `--manpage` | Print the comprehensive Markdown reference manual and exit |
| `--print-palettes` | Print built-in theme palettes as ANSI color cards and exit |
| `-h`, `--help` | Print command-line help |
| `-V`, `--version` | Print the AICS version |

`--manpage`, `--write-rules-dts`, `--delete-index`, and `--print-palettes` exit without
